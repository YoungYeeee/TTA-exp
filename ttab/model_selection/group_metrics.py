...
class GroupLossComputer:
    """
    This class is used to compute group-wise metrics in datasets such as Waterbirds.
    Modify https://github.com/kohpangwei/group_DRO/blob/master/loss.py
    ...
    def get_target_metrics(self, stats_dict, group_counts):
        metrics = {}
        loss_metrics = []
        acc_metrics = []
        ...
        ratio = group_counts / np.sum(group_counts)
        metrics["robust_loss"] = np.max(
            stats_df.loc[:, loss_metrics].values, axis=1
        ).item()
        metrics["robust_acc"] = np.min(
            stats_df.loc[:, acc_metrics].values, axis=1
        ).item()
        metrics["avg_loss"] = (stats_df.loc[:, loss_metrics].values @ ratio).item()
        metrics["avg_acc"] = (stats_df.loc[:, acc_metrics].values @ ratio).item()
        return metrics

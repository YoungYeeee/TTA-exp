...
def _augmix_aug_cifar(x_orig: torch.Tensor, data_name: str) -> torch.Tensor:
...
def tr_transforms_cifar(image: torch.Tensor, data_name: str) -> torch.Tensor:
    """
    ...
    scale_size = 36 if input_size == 32 else 256  # input size is either 32 or 224
    padding = int(scale_size - input_size)
    tensor_to_image, preprocess = get_ops(data_name)

...
class MergeMultiTestDatasets(object):
    ...
    #     #print('MergeMultiTestDatasets',indices)
    ...
    #     #dataset.replace_indices(indices_pattern="new", new_indices=indices)
    #     #targets = [
    #     #    targets[i] for i in indices

    #     #]  # use indices to get the targets of interest.
    #     #print('MergeMultiTestDatasets')
    #     #print(indices[:10])
    #     #print(targets[:20])
    #     new_indices = []
    #     #print(3333)
    ...
    def merge(
        ...
        if isinstance(test_case.inter_domain, HomogeneousNoMixture):
        ...
        elif isinstance(test_case.inter_domain, CrossHeterMixture):
            # data = self._intra_shuffle_dataset(
            #     self._merge_datasets(test_datasets), random_seed=random_seed
            # )
            print(test_datasets)
            # data =  self._merge_datasets(test_datasets)
            data = self._intra_shuffle_dataset(
                 self._merge_datasets(test_datasets), random_seed=random_seed
             )
            # # print(data)
            # indices = data.query_dataset_attr("indices")
            # print(indices[:10])
            # targets = data.query_dataset_attr(
            #     "targets"
            # )  # targets are always the same, which is the original label list.
            #
            # targets = [
            ...
            # print(data)
            # print(targets)
            data2 = self._intra_non_iid_shift(
                        dataset=data,
                        non_iid_pattern=test_case.inter_domain.non_iid_pattern,
                        non_iid_ness=test_case.inter_domain.non_iid_ness,
                        random_seed=random_seed,
                    )
            indices = data2.query_dataset_attr("indices")
            targets = data2.query_dataset_attr(
                "targets"
            )  # targets are always the same, which is the original label list.
...
class ConstructTestDataset(object):
    ...
    def _get_cifar_test_domain_datasets_helper(
        ...
        print(os.path.join(self.data_path, self.base_data_name))
        return dataset

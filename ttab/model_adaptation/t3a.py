...
class T3A(BaseAdaptation):
    ...
    def select_supports(self):
        ...
        indices1 = indices1.to(self._meta_conf.device)
        
        for i in range(self._num_classes):
            _, indices2 = torch.sort(ent_s[y_hat == i])
            indices2 = indices2.to(self._meta_conf.device)
            
            indices1_i = indices1[y_hat == i]
            indices2_top_M = indices2[:top_M]
            indices.append(indices1_i[indices2_top_M])

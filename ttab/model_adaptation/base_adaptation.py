...
class BaseAdaptation(object):
    ...
    def initialize(self, seed: int):
        ...
        if self._meta_conf.model_selection_method == "oracle_model_selection":
            ...
            # record optimal adaptation steps for each batch
    ...
    def compute_fishers(self, scenario, data_size):
        ...
        self.fisher_dataset, self.fisher_loader = self.get_src_data(
            scenario, data_size)
        ...
        for step, _, batch in self.fisher_loader.iterator(
            ...
            for name, param in self._model.named_parameters():
                if param.grad is not None:
                    if step > 1:
                        fisher = (
                            param.grad.data.clone().detach() ** 2 +
                            fishers[name][0]
                        )
                    else:
                    ...
                    if step == len(self.fisher_dataset):
                        fisher = fisher / step
                    fishers.update(
                        {name: [fisher, param.data.clone().detach()]})
